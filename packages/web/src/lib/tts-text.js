...
//
// 调用链（tts-store.tick 里）：
//   textBuf ──cutBlock──→ { text, raw, rest } ──→ tts.speak
...
//   满 400 分起：往后碰到的第一个句末标点就是切点；一直没标点则满 800 分硬切；不足 400 分整段发。
//   400 分 ≈ 100 中文字 ≈ 400 英文字母 → 两种语言读出来时长齐平
...
//   念错一个标点无所谓，漏念半句话就是事故。
//
// 代价（认了）：跨块的 markdown 洗不干净。代码围栏最典型 —— 流式一行行来，
//   配不出成对的 ```…```，所以**只删 ``` 标记本身，代码内容照念**（已与用户确认）。
