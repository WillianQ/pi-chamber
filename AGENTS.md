...
| `tts.end` | emit(s→w) | `{reason:"done"\|"cancelled"\|"error", error?}` | — | 会话终结，**必到一次**（会话收束后会话残留清零）；error 前端只记状态、有缓存照常续喂。★ **阿里侧 task-failed / 连接断【不发】这个帧** —— 后端自愈重开 task，前端无感；只有 ① 阿里回 task-finished ② 前端 finish 而 task 已没了（补回执）③ **连吃 3 次失败且一次声都没出过**（key 无效等真故障）才发 |
...
- **TTS 会话模型**（语音输出）：★ **会话与阿里 task 是两层** —— 会话（前端眼中）自首个 `tts.speak` 起、至 `tts.end` 止；阿里 task 只是它的载体，有文本就开、没了就丢，**可随时重建而会话不中断**。会话中再 speak = 阿里 duplex continue-task 追加（后端自动切段，单次 ≤20000 字符），收尾中拒接。rate/voice 随 speak 携带、run-task 定死 —— **task 重建时重新现读**（改设置立即生效，不用等下个会话）。
  **自愈全在后端**（前端当"永不掉线"用）：阿里 task-failed / 连接断 → **不发 `tts.end`**，只清 task + 未确认文本回待发队列 → 下次 speak 懒重连 + 重开 task。实测依据：阿里 23s 收不到 continue 就报 `request timeout after 23 seconds.`（**是 continue 间隔，与是否在推音频无关**），但它**推音频约 10 倍实时**（38.5s 音频 4s 推完）→ 超时前已收文本早念完了，**超时本身不丢音频**；真正会丢的只有"刚受理还没交到阿里"的那一小段，那正是文本账本要兜的（未出声断线整段重放 / 已出声只补没发出去的）。浏览器断线清在途会话。与 STT 各持一条阿里面向 WS（同 socket 只能跑一个在途任务）；实现唯一真相 = tts-service.js 文件头注。
